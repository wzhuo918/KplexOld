...
//			if(true)
			{
				...
				HashMap<Integer, Pair> not = new HashMap<Integer, Pair>();
				// 状态栈，记录所有状态
				...
				while (!stack.isEmpty())// 状态栈不空时执行操作
				{
					...
					while (curRes.size() + curCand.size() >= quasiCliqueSize) {
						...
						if(isCritEmpty||prunableNot.isEmpty()){
							...
							Pair minp = null;int mind = curCand.size()+1;
							for(Pair n:prunableNot){
								if(n.cdeg<mind){
									mind = n.cdeg;
									minp = n;
								}
							...
							if(mind==0)
								...
								HashSet<Integer> set = oneLeap.get(minp.point);
						...
						if(!isCritEmpty&&!prunableNot.isEmpty()){
							...
							for(Pair n:prunableNot){
								if(!set.contains(n.point))
									n.cdeg--;
							}
						}
						HashMap<Integer, Integer> tmpcand = (HashMap<Integer, Integer>) curCand
								.clone();
						HashMap<Integer, Pair> tmpnot = (HashMap<Integer, Pair>) curNot
								.clone();
...
//							Pair newnp = new Pair(vp,counter);
						curNot.put(vp, new Pair(vp,counter));
						connnot.add(new Pair(vp,counter));
					}
				}
			}
		}
	 ...
	 */
	private static HashSet<Pair> getPrunableNot(HashSet< Pair> curNot) throws CloneNotSupportedException {
		HashSet<Pair> re = new HashSet<Pair>();
		for(Pair p:curNot){
			if(p.rdeg==0)
				re.add(p.clone());
		}
	 ...
	 * @param curCand 候选集
	 */
	private static void updateNotCdeg(HashMap<Integer, Pair> curNot,
			HashMap<Integer, Integer> curCand) {
		for(Pair p:curNot.values()){
			p.cdeg = countNotAdj(curCand,p.point);
		}
	}
	...
	private static void updatecountp(HashMap<Integer, Pair> curCand,
			Integer vp) {
		HashSet<Integer> adj = oneLeap.get(vp);
		int num = 0;
		Iterator<Map.Entry<Integer, Pair>> ite = curCand.entrySet()
				.iterator();
		while (ite.hasNext()) {
...
//				crit.add(en.getKey());
	...
	private static HashSet<Integer> getConnSet(
			...
			HashSet<Pair> connnot,HashMap<Integer, Pair> curNot) {
		HashSet<Integer> all = new HashSet<Integer>();
		...
		if(curNot.size()<all.size()){
			for(Entry<Integer,Pair>n:curNot.entrySet()){
				if(all.contains(n.getKey()))
					connnot.add(n.getValue());
			}
		}else{
			for(Integer a:all){
				Pair n = curNot.get(a);
