...
public class MyOptimize {
				...
				while (!stack.isEmpty())// 状态栈不空时执行操作
				{
					...
					while (curRes.size() + curCand.size() >= quasiCliqueSize) {
						...
						if (!isCritEmpty && !prunableNot.isEmpty()) {
							...
							for (Entry<Integer, Integer> n : prunableNot
									.entrySet()) {
								if (!set.contains(n.getKey()))
									n.setValue(n.getValue() - 1);
							}
						}
						HashMap<Integer, Integer> tmpcand = null;
						HashMap<Integer, Integer> tmpnot = null;
						...
						if (cirtset.size() > 0) {
							...
							tmpcand = filterUnCrit(curCand, intersection);
							tmpnot = filterUnCrit(curNot, intersection);
						} else {
							tmpcand = (HashMap<Integer, Integer>) curCand
									.clone();
							tmpnot = (HashMap<Integer, Integer>) curNot.clone();
						}
	...
	private static HashMap<Integer, Integer> filterUnCrit(
			HashMap<Integer, Integer> curCand, HashSet<Integer> intersection) {
		HashMap<Integer, Integer> re = new HashMap<Integer, Integer>();
		if (curCand.size() < intersection.size()) {
			for (Entry<Integer, Integer> en : curCand.entrySet()) {
				if (intersection.contains(en.getKey()))
					re.put(en.getKey(), en.getValue());
			}
		} else {
			for (Integer i : intersection) {
				Integer v = curCand.get(i);
				if (v != null) {
					re.put(i, v);
				}
			}
	...
	private static int countNotAdj(HashMap<Integer, Integer> curCand, Integer vp) {
		int num = 0;
	 ...
	 */
	private static HashMap<Integer, Integer> getPrunableNot(
	 ...
	 */
	private static boolean isConnSetEmpty(
	...
	/**
	 * 初始化配置信息,包括取计算点的间隔reduceNumber, 有意义的最小kplex大小quasiCliqueSize, kplex 的k值
	 * 
	 * @param args
